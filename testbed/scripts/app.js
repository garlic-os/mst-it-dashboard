...
var welcomeVue = new Vue({
	...
	methods: {
		...
		validateUser: function(nickname, cb) {
			...
			fbRef.ref('Users/' + nickname).once("value", function(snapshot){
				// Failure to login
				if(!snapshot.exists()) {
					cb(false);
				}
		...
		regUser: function() {
			...
			fbRef = firebase.database();
			fbRef.ref('Users/' + self.userNickname).set({
		...
		postLogin: function() {
			...
			fbRef.ref('Users/' + store.user.nickname + '/lastActive').set(Date.now());
			self.$emit('login');
		}
	}
});
...
var newQueVue = new Vue({
	...
	methods: {
		...
		newQSubmit: function() {
			...
			var newQ = fbRef.ref('Questions/' + self.catOption).push();
			newQ.set({
				text: self.formText,
				options: [self.formOpt1, self.formOpt2, self.formOpt3, self.formOpt4],
				author: self.user.fullname
			});
			fbRef.ref('Answers/' + newQ.key()).set(self.formCorrect);
			fbRef.ref('Users/' + self.user.nickname + '/questions').once("value", function(snap) {
				var newQnum = snap.exists() ? snap.val() + 1 : 1;
				fbRef.ref('Users/' + self.user.nickname + '/questions').set(newQnum, function(){
					toast('Thanks for contributing!');
				});
			});
			fbRef.ref('Users/' + self.user.nickname + '/score').once("value", function(snap){
				var newScore = snap.val() + 25;
				fbRef.ref('Users/' + self.user.nickname + '/score').set(newScore);
			})
			store.updateState("navigation");
			self.reset();
		},
...
var navigationVue = new Vue({
	...
	methods: {
		loadNav: function() {
			...
			var self = this;
			// Create User List
			fbRef.ref('Users').orderByChild('lastActive').limitToLast(50).on("child_added", function(snap){
			...
			// Track the matches
			fbRef.ref('Users/' + self.user.nickname + '/matches').on('child_added', function(snap){
				self.matches[snap.val().opponent] = {id: snap.key()};

				//Only track currentTurn
				fbRef.ref('Matches/' + snap.key() + '/currentTurn').on("value", function(matchChangeSnap){
			...
			fbRef.ref('Users/' + self.user.nickname + '/matches').on('child_removed', function(snap){
				self.$set('users.' + snap.val().opponent + '.subText', '');
				delete self.matches[snap.val().opponent];
			});
			// Display Notifications
			fbRef.ref('Notifications/' + self.user.nickname).on("child_added", function(snap){
				fbRef.ref('Notifications/' + self.user.nickname + '/' + snap.key()).remove();
				toast(snap.val().message);
			});
			// Track Leaders
			fbRef.ref('Users').orderByChild('score').limitToLast(25).on("value", function(snap){
	...
	methods: {
		loadQuestion: function(who, matchID) {
			...
			self.getQuestionCategory(category, function() {
				...
				// Get the question
				fbRef.ref('Questions/' + category + '/' + Qid).once('value', function(snap){
		...
		getQuestionCategory: function(category, cb) {
			...
			if (!(category in self.categoryTimeTable) || (Date.now() - self.categoryTimeTable[category]) > 60000) {
				ajaxGet("https://it-trivia-and-it-help-settings.firebaseio.com/Questions/"+ category + ".json?shallow=true", function(data) {
		...
		checkAnswer: function() {
			...
			if(!window.localStorage.getItem(self.match.id) || (Date.now() - window.localStorage.getItem(self.match.id)) / 1000 > 40) {
				...
				fbRef.ref('Answers/' + self.match.question).once('value', function(ansSnap){
					if (ansSnap.val() == self.selection) {
						fbRef.ref('Matches/' + self.match.id + '/' + self.user.nickname).once("value", function(matchSnap){
							var newScore = matchSnap.exists() ? matchSnap.val() +  1 : 1;
							fbRef.ref('Matches/' + self.match.id + '/' + self.user.nickname).set(newScore, function(){
		...
		createMatch: function(who) {
			var self = this;
			var matchRef = fbRef.ref('Matches').push();
			self.match.id = matchRef.key();
			self.match.opponent = who;
			fbRef.ref('Users/' + self.user.nickname + '/matches/' + self.match.id).set({opponent: who});
			fbRef.ref('Users/' + who + '/matches/' + self.match.id).set({opponent: self.user.nickname});
			fbRef.ref('Notifications/' + who).push().set({message: self.user.fullname.split(" ")[0] + " has initiated a match with you! Good Luck"});
			matchRef.set({currentTurn: self.user.nickname});
			self.loadQuestion(who, self.match.id);
		},
		updateMatch: function() {
			var self = this;
			fbRef.ref('Matches/' + self.match.id + '/numAsked').once("value", function(askedSnap) {
				var numQasked = askedSnap.exists() ? askedSnap.val() + 1 : 1;
				// Can we determine a Winner?
				if (numQasked >= 8 && numQasked % 2 === 0) {
					fbRef.ref('Matches/' + self.match.id).once("value", function(matchSnap){
						// You Won
						...
						// Tied
							...
							fbRef.ref('Matches/' + self.match.id).update({currentTurn: self.match.opponent, numAsked: numQasked});
							fbRef.ref('Notifications/' + self.match.opponent).push().set({
		...
		updateScores: function(winner, losser) {
			...
			fbRef.ref('Users/' + winner).once('value', function(snap){
				var newScore = snap.val().score ? snap.val().score + 100 : 100;
				var newGamesPlayed = snap.val().gamesPlayed ? snap.val().gamesPlayed + 1 : 1;
				fbRef.ref('Users/' + winner).update({score: newScore, gamesPlayed: newGamesPlayed});
			});
			fbRef.ref('Users/' + losser + '/gamesPlayed').once('value', function(snap){
				var newGamesPlayed = snap.exists() ? snap.val() + 1 : 1;
				fbRef.ref('Users/' + losser + '/gamesPlayed').set(newGamesPlayed);
			});
			fbRef.ref('Matches/' + self.match.id).remove();
			fbRef.ref('Users/' + winner + '/matches/' + self.match.id).remove();
			fbRef.ref('Users/' + losser + '/matches/' + self.match.id).remove();
		}
	}
});
...
var reviewVue = new Vue({
	...
	methods: {
		...
		flagQuestion: function(e) {
			var self = this;
			e.preventDefault();
			fbRef.ref('Flags/' + self.question + '/' + e.target.textContent).push().set(true);
	...
	methods: {
		...
		handleAnalyzeUser: function() {
			...
			var url = /^\d+$/.test(this.analyzeUser) ? "https://itweb.mst.edu/auth-cgi-bin/cgiwrap/analyzer/analyze.pl?mode=search_emplid&emplid="+this.analyzeUser+"&which=all" :
													   "https://itweb.mst.edu/auth-cgi-bin/cgiwrap/analyzer/analyze.pl?mode=view&userid="+this.analyzeUser+"&which=all";
			this.newIframe("Analyze User", url);
			this.analyzeUser = "";
		},
		...
		handlekbExternal: function() {
			this.newIframe("Knowledge Base - EXTERNAL", "https://kb.mst.edu/search.php?q=" + this.passReset);
			this.passReset = "";
		},
		handleAdminPass: function() {
			window.open("https://password.umsystem.edu/MSTsupport");
		},
		handleSccmOs: function() {
			this.newIframe("Web Template Generator - OS Install","https://itweb.mst.edu/auth-cgi-bin/cgiwrap/deskwtg/generate.pl?mode=search&platform=win10-x64-sccm2012&host=" + this.sccm);
			this.sccm = "";
		},
		handleSccmView: function() {
			this.newIframe("Web Template Generator - View Templates","https://cmdesk-p1.srv.mst.edu/ReportServer_RDP/Pages/ReportViewer.aspx?%2fConfigMgr_RDP%2f00+Desktop+Engineering%2fMST+All+task+sequence+deployments");
			this.sccm = "";
		},
		...
		handleDell: function() {
			...
			// Instead we'll just open a new tab to Dell
			window.open("http://www.dell.com/support/home/us/en/19/product-support/servicetag/" + this.dell);
...
quizVue.$on('game_over', function(winner, losser) {
	...
	fbRef.ref('Notifications/' + winner).push().set({message: "You beat " + losserFirstName});
	fbRef.ref('Notifications/' + losser).push().set({message: "You lost to " + winnerFirstName});
});
