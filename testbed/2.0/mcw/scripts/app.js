...
var mainVue = new Vue({
	...
	methods: {
		...
		handlePortSearch: function() {
			var self = this;
			self.foundBuildings = [];
			if (self.building.length === 0)
				return;
			self.buildings.forEach(function(building) {
				if (building.search(self.building) !== -1 || building.search(self.building) !== -1){
					self.foundBuildings.push(building);
				}
			});
		},
		handlePortSelect: function(e) {
			this.building = e.target.textConent;
			this.foundBuildings = [];
		},
		handlePort: function() {
			if(this.building === null || this.portJack === null) {
				if(this.building === null){
					this.newIframe("Port Search - Jack Only", "https://itweb.mst.edu/auth-cgi-bin/cgiwrap/swconf/user-port-search.pl?mode=search&jack=" + this.portJack);
				}
				else{
					this.newIframe("Port Search - Building Only", "https://itweb.mst.edu/auth-cgi-bin/cgiwrap/swconf/user-port-search.pl?mode=search&location=" + this.building);
				}
			}
			else{
				this.newIframe("Port Search", "https://itweb.mst.edu/auth-cgi-bin/cgiwrap/swconf/user-port-search.pl?mode=search&location=" + this.building + "&jack=" + this.portJack);
			}
			this.building = "";
			this.jackPort = "";
		},
